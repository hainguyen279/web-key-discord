...
async function requestKeyFromBot(){
  try{
    // CUNG DOMAIN voi bot (duoc bot phuc vu qua express.static) - dung
    // duong dan TUONG DOI, khong can API_BASE_URL/CORS nua.
...
// Luu KEM NGAY (theo gio Viet Nam, UTC+7) luc tao key - dung de tu dong
// het han key HIEN THI o web sang ngay hom sau, bat lay key moi. Dinh
// dang "YYYY-MM-DD" tinh theo gio VN, KHONG phai gio may nguoi dung.
const KEY_DATE_STORAGE=STORED_KEY+'_date';
function vnDateString(){
  // +7h roi lay ngay theo UTC de tranh phu thuoc timezone cua trinh duyet
  const vnMs=Date.now()+7*60*60*1000;
  return new Date(vnMs).toISOString().slice(0,10); // "YYYY-MM-DD"
}
function getStoredKey(){
  const key=localStorage.getItem(STORED_KEY)||'';
  if(!key) return '';
  const savedDate=localStorage.getItem(KEY_DATE_STORAGE)||'';
  if(savedDate!==vnDateString()){
    // Sang ngay moi (gio VN) - key cu bi coi la het han TREN WEB (khong
    // lien quan gi den han kich hoat that ben Discord), buoc lay key moi.
    localStorage.removeItem(STORED_KEY);
    localStorage.removeItem(KEY_DATE_STORAGE);
    return '';
  }
  return key;
}
function saveKeyWithDate(key){
  ...
  localStorage.setItem(KEY_DATE_STORAGE,vnDateString());
}
